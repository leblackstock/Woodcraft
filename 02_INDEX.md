# 02 Index

Master map of this workspace. Start with [00_START_HERE.md](00_START_HERE.md).

## Core Orientation and Rules

- [README.md](README.md) — repo purpose, quick start, source-of-truth links.
- [00_START_HERE.md](00_START_HERE.md) — exact reading order and startup rules.
- [01_VISION.md](01_VISION.md) — mission, constraints, success definition.
- [02_INDEX.md](02_INDEX.md) — top-level map of files/folders grouped by function.
- [03_GOVERNANCE.md](03_GOVERNANCE.md) — precedence, editing rules, AI behavior rules.
- [04_BUSINESS_RULES.md](04_BUSINESS_RULES.md) — hard budget/profit/risk guardrails.
- [14_GLOSSARY.md](14_GLOSSARY.md) — stable definitions to prevent drift.

## Strategy and Workflow

- [05_CHANNEL_STRATEGY.md](05_CHANNEL_STRATEGY.md) — channel jobs and boundaries.
- [06_PRODUCT_DECISION_WORKFLOW.md](06_PRODUCT_DECISION_WORKFLOW.md) — intake to approval workflow.
- [07_LISTING_AND_CONTENT_WORKFLOW.md](07_LISTING_AND_CONTENT_WORKFLOW.md) — production-to-posting workflow.
- [08_AUTOMATION_ROADMAP.md](08_AUTOMATION_ROADMAP.md) — automation phases and dependencies.
- [11_WEEKLY_OPERATIONS.md](11_WEEKLY_OPERATIONS.md) — weekly cadence and low-energy fallback plan.

## Data, Prompts, and Execution Tracking

- [09_DATA_MODEL.md](09_DATA_MODEL.md) — record schemas and source-of-truth notes.
- [10_PROMPTS_INDEX.md](10_PROMPTS_INDEX.md) — prompt families, naming, and versioning plan.
- [12_DECISION_LOG.md](12_DECISION_LOG.md) — key decisions and rationale.
- [13_BACKLOG.md](13_BACKLOG.md) — prioritized next actions.

## Historical Operating Records

- [90_archive/weekly_reviews/](90_archive/weekly_reviews/) — completed weekly review instance files stored by date.

## Working Folders

- [20_research/](20_research/) — market research snapshots, competitor notes, pricing observations.
- [30_products/](30_products/) — product candidates, approved items, cost/build notes.
- [40_listings/](40_listings/) — listing drafts, published records, variant tests.
- [50_content/](50_content/) — social plans, caption drafts, seasonal campaigns.
- [60_automation/](60_automation/) — workflow automation specs and phased implementation notes.
- [70_ads/](70_ads/) — tiny-budget ad experiments and learnings.
- [80_templates/](80_templates/) — reusable markdown templates for execution.
- [90_archive/](90_archive/) — deprecated docs, retired plans, and completed weekly review logs.
