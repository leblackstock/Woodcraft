# Woodcraft AI Marketing Workspace

## Local Research UI

- [http://127.0.0.1:8765](http://127.0.0.1:8765)

This repository is a markdown-first operating system for selling handmade cedar decor, planters, raised beds, and outdoor furniture with AI support.

It is designed to reduce decision fatigue, keep actions consistent, and prioritize cash-flow outcomes for a low-budget business.

## Dual-Model Operating Note

- GPT-5.5 handles workflow orchestration, preparation, validation, and queue movement.
- Claude is the only approved writer of final customer-facing copy.
- No customer-facing listing, caption, reply template, or promo asset is publish-ready until the Claude copy pass is complete.

## Start Here

1. Read [00_START_HERE.md](00_START_HERE.md)
2. Read [01_VISION.md](01_VISION.md)
3. Read [03_GOVERNANCE.md](03_GOVERNANCE.md)
4. Read [04_BUSINESS_RULES.md](04_BUSINESS_RULES.md)
5. Use [02_INDEX.md](02_INDEX.md) to navigate everything else

## Source of Truth Files

- [00_START_HERE.md](00_START_HERE.md) — session orientation + reading order
- [01_VISION.md](01_VISION.md) — mission, priorities, channel strategy summary
- [03_GOVERNANCE.md](03_GOVERNANCE.md) — operating rules and decision hygiene
- [04_BUSINESS_RULES.md](04_BUSINESS_RULES.md) — hard budget/profit/risk guardrails
- [05_CHANNEL_STRATEGY.md](05_CHANNEL_STRATEGY.md) — what each platform is for

## Core Workflows

- [06_PRODUCT_DECISION_WORKFLOW.md](06_PRODUCT_DECISION_WORKFLOW.md)
- [07_LISTING_AND_CONTENT_WORKFLOW.md](07_LISTING_AND_CONTENT_WORKFLOW.md)
- [11_WEEKLY_OPERATIONS.md](11_WEEKLY_OPERATIONS.md)

## Planning, Data, and Execution Support

- [08_AUTOMATION_ROADMAP.md](08_AUTOMATION_ROADMAP.md)
- [09_DATA_MODEL.md](09_DATA_MODEL.md)
- [10_PROMPTS_INDEX.md](10_PROMPTS_INDEX.md)
- [12_DECISION_LOG.md](12_DECISION_LOG.md)
- [13_BACKLOG.md](13_BACKLOG.md)
- [14_GLOSSARY.md](14_GLOSSARY.md)
- [80_templates/](80_templates/)

For full navigation, use [02_INDEX.md](02_INDEX.md).
