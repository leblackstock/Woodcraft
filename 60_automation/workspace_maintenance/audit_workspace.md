...
Read-only scan of live Markdown for broken relative links, references to currently retired files, UTF-8 decoding artifacts in editable workspace files, and exact repeated long guidance in important workflow files. It also traces each changed eligible workflow document through its live linked package. Immutable raw source packages are outside encoding reporting.
...
- Package checks: broken links, active references to Deprecated/Retired/Superseded files, removed workflow documents needing lifecycle review, and exact non-intentional repeated guidance. The trace does not infer semantic contradictions from free-form prose.
